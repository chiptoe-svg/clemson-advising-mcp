...
| `check-instructor-conflicts`  | Which of a list of instructors teach during a day/time window                                                          | Three-state per person: `busy` / `free` / `not_teaching` — the last is NOT free, it means the snapshot has no sections for them. Emails match exactly; an ambiguous name returns candidates. Teaching conflicts only.                                                                                                  |
