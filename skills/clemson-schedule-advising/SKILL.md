...
| `get-course-offerings`        | Which terms each course actually ran (section counts) + per-season offered-of-observed rollups — offering-pattern evidence | Read `observed_terms` FIRST: an unobserved term is UNKNOWN, never "not offered"; observed-but-absent means it did not run that term. Old course codes keep their history. Batch all of a plan's courses into one call.                                                                                    |
