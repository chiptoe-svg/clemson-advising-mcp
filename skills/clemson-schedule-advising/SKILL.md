...
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `search-classes`              | Search by subject and/or course number, with instructor / building-room / days / time / open-seats filters             | **Must** scope by `subject` or `course_number` — an unscoped search is rejected. Snapshot-backed (refreshed ~05:00); pass `refresh: true` only for live seat counts, and only after scoping. **If the response carries `truncated`, the list is PARTIAL and biased** — it is the sections with the MOST OPEN SEATS, so the fullest are the ones missing. Never answer "what was offered", "how many", or any demand question from a truncated page: narrow (`course_number`, `days`, `instructor`) or page with `offset` until you hold all `totalCount`. (A real miss: asked for Summer 2026's GC 4000-level courses, a model reported 2 of 18 from a truncated page.) Sections carry `partOfTerm` verbatim — `1` full term, `H1`/`H2` = Summer 1/Summer 2, `MMA`..`MMD` = mini-mesters; `null` means the snapshot predates the field, NOT that the course had no session. Instructor and room are filters here, not separate tools. |
...
| `get-course-facts`           | Catalog facts for MANY courses at once (<=200): credits, title, prereq text+parse, coreq parse, structured coreqs | Batch every course of a plan into ONE call instead of looping `get-course-details`. `found: false` = the catalog has no such course (an absence, not an outage; outages error the whole call). Course rows are the current inventory, not catalog-year-pinned. |
...
| `get-course-offerings`        | Which terms each course ran (counts) + per-season evidence and `estimated_probability` with label — offering patterns | Read `observed_terms` FIRST: an unobserved term is UNKNOWN, never "not offered"; observed-but-absent means it did not run that term. Old course codes keep their history. Batch all of a plan's courses into one call. Quote `estimated_probability` to a student only as an estimate from history — the registrar has promised nothing. EXCEPTION: a season with `known_decision` (label `ruled out`/`confirmed`) is a recorded departmental decision and overrides the estimate — give THAT as the answer, citing its note/source.                                                                                    |
| `get-course-enrollment-history` | ENROLLMENT + capacity per course per term (<=200 courses, <=50 terms) — demand and capacity planning | Batch the whole question into ONE call; the sibling `get-course-offerings` has counts but no enrollment. CHECK each observed term's `status`: `pre_term` means the snapshot predates the term, so its numbers are registration-in-progress, NOT comparable demand — charting them beside `final` terms invents a cliff. `fill_rate` above 1.0 is real (labs run over cap). Unobserved term = unknown, never zero demand. |
...
| `get-degree-requirements`   | Requirements as the REGISTRAR states them (Degree Works), for a program + catalog year                                             | A DIFFERENT PROVENANCE from `get-program-plan`: the catalog says what to take and when, this says what actually SATISFIES each requirement. Note `unit` — 3 credits and 1 class differ — and `conjunction` ('and' = take all). `alternatives` are whole routes: satisfy ONE. An empty list means no audit imported, NEVER a degree without requirements. |
...
| `list-courses`              | Courses by subject and/or catalog-number range — resolves DegreeWorks-style wildcards (`MGT @`, `ACCT 3000:3999`)     | Reads the CURRENT course inventory; rows are not catalog-year-pinned and the response says so. Requires at least one filter.  |
