...
function validate(value: unknown, schema: Schema, path = "$"): string[] {
  ...
  if (schema.type !== undefined && !typeOk(value, schema.type)) {
    errs.push(
      `${path}: expected ${JSON.stringify(schema.type)}, got ${Array.isArray(value) ? "array" : value === null ? "null" : typeof value}`,
    );
  ...
  if (schema.type === "object" || schema.properties) {
    ...
    for (const key of schema.required ?? []) {
      ...
      if (obj[key] === undefined)
        errs.push(`${path}.${key}: required but absent`);
    }
    for (const [key, sub] of Object.entries(schema.properties ?? {})) {
      if (obj[key] !== undefined)
        errs.push(...validate(obj[key], sub as Schema, `${path}.${key}`));
    }
  }
  if (schema.type === "array" && schema.items) {
    (value as unknown[]).forEach((el, i) =>
      errs.push(...validate(el, schema.items, `${path}[${i}]`)),
    );
  }
  return errs;
}
...
test("okJson emits structuredContent AND the text block, and they agree", () => {
  ...
  assert.deepEqual(
    r.structuredContent,
    data,
    "structuredContent must carry the object",
  );
  const text = (r.content as Array<{ text: string }>)[0]!.text;
  assert.deepEqual(
    JSON.parse(text),
    data,
    "text block must still carry the same payload",
  );
});
...
test("okJson stays backward compatible: text is always present", () => {
  for (const payload of [{ a: 1 }, [1], "s", 3, null]) {
    ...
    assert.ok(
      Array.isArray(r.content) && r.content.length === 1,
      "text content must always be emitted",
    );
  }
});
...
async function callStructured(
  ...
  assert.ok(
    r.structuredContent,
    "handler must return structuredContent when it declares an outputSchema",
  );
  return r.structuredContent as Record<string, unknown>;
}

test(
  "find-course-in-program conforms to its outputSchema — found case",
  ...
  async () => {
    ...
    assert.deepEqual(
      validate(out, schema),
      [],
      "found-case response must conform",
    );
...
test(
  "find-course-in-program conforms to its outputSchema — not-found case",
  { skip: SKIP_NO_CORE_DB },
  async () => {
    const schema = findCourseInProgram.tool.outputSchema as Schema;
    const out = await callStructured(findCourseInProgram, {
    ...
    assert.deepEqual(
      validate(out, schema),
      [],
      "not-found response must conform",
    );
...
test("the validator actually rejects a non-conforming payload", () => {
  ...
  assert.notDeepEqual(
    validate({}, schema),
    [],
    "missing required keys must fail",
  );
  assert.notDeepEqual(
    validate(
      {
