...
function python(args: string[]): unknown {
  try {
    const out = execFileSync(
      GC_ADVISOR_PYTHON,
      [GC_ADVISOR_QUERY, "--db", CATALOG_DB, ...args],
      {
        encoding: "utf-8",
        timeout: 30_000,
      },
...
function assertSame(node: unknown, py: unknown, what: string): void {
  assert.deepEqual(
    normalise(node),
    normalise(py),
    `${what}: Node output differs from Python`,
  );
}
...
test(
  "getProgramPlan matches query.py for EVERY program x catalog year",
  { skip: SKIP },
  () => {
    const db = openCatalog(CATALOG_DB);
    try {
      ...
      for (const { name, year } of pairs) {
        ...
        assertSame(
          getProgramPlan(db, year, name),
          py,
          `program-plan ${name} ${year}`,
        );
        compared++;
      }
...
test(
  "getRequirementRules matches query.py for EVERY program x catalog year",
  { skip: SKIP },
  () => {
    const db = openCatalog(CATALOG_DB);
    try {
      ...
      for (const { name, year } of pairs) {
        ...
        assertSame(
          getRequirementRules(db, year, name),
          py,
          `req-rules ${name} ${year}`,
        );
        compared++;
...
test(
  "getCourse matches query.py across a sample of real course codes",
  { skip: SKIP },
  () => {
    const db = openCatalog(CATALOG_DB);
    try {
      const codes = (
        db
          .prepare("SELECT code FROM course ORDER BY code LIMIT 40")
      ...
      assertSame(
        getCourse(db, "ZZZZ 9999"),
        python(["course", "--code", "ZZZZ 9999"]),
        "course ZZZZ 9999",
      );
    } finally {
      db.close();
...
test(
  "an unknown year and an unknown program both throw, as Python raises KeyError",
  { skip: SKIP },
  () => {
    const db = openCatalog(CATALOG_DB);
    try {
      assert.throws(
        () => getProgramPlan(db, "1899-1900", "Anything"),
        /Unknown catalog year/,
      );
      const year = listCatalogYears(db)[0]!;
      assert.throws(
        () => getProgramPlan(db, year, "No Such Program"),
        /No program/,
      );
      assert.throws(
        () => getRequirementRules(db, year, "No Such Program"),
        /No program/,
      );
    } finally {
      db.close();
...
test(
  "the differential harness would actually catch a difference",
  { skip: SKIP },
  () => {
    ...
    assert.doesNotThrow(() =>
      assertSame({ a: 1, b: 2 }, { b: 2, a: 1 }, "sanity"),
    );
  },
);
...
test(
  "the live catalog DB has no stale bogus flags",
...
test(
  "the tripwire agrees with Python's is_bogus_rule on the live corpus",
  { skip: SKIP },
  () => {
