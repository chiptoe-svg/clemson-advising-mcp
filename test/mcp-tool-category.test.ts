...
import { registerTools, __registeredToolsForTest } from "../src/mcp-tools/server.ts";
import "../src/mcp-tools/index-public.ts";
...
test("a registered tool's category round-trips through client.listTools() as _meta", async () => {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: __registeredToolsForTest().filter((t) => t.tool.name === "search-classes").map((t) => t.tool),
  }));
  ...
  assert.ok(tool, "search-classes registered (import the public barrel in this test)");
  assert.equal((tool!._meta as Record<string, unknown>)[TOOL_CATEGORY_META_KEY], "core");
  await client.close(); await server.close();
});

test("registerTools refuses a definition without a category", () => {
  const before = __registeredToolsForTest().length;
  registerTools([{ operation: "clemson.search_classes", tool: { name: "no-cat", inputSchema: { type: "object", properties: {} } }, handler: async () => ({ content: [] }) } as never]);
