...
test("finds a course that exists ONLY in the requirement rules", { skip: SKIP_NO_CORE_DB }, async () => {
  const { json } = await find({ course: "PHYS 1220", program: GC, catalog_year: YEAR });
  assert.equal(json.found, true, "a rules-only course must be found");
  assert.deepEqual(json.plan_appearances, [], "PHYS 1220 is not in the semester plan");
  assert.equal(json.requirement_rule_mentions.length, 1, "it IS in a requirement rule");
  assert.equal(
  ...
  assert.equal(json._note, undefined, "a found course must carry NO absence note");
});

test("a rules-only hit still conforms to the mention shape", { skip: SKIP_NO_CORE_DB }, async () => {
...
test("a subject prefix matches across BOTH stores at once", { skip: SKIP_NO_CORE_DB }, async () => {
  // PHYS appears only in rules for this program-year; the prefix path must
  // reach the rules half too, not just the plan half.
  const { json } = await find({ course: "PHYS", program: GC, catalog_year: YEAR });
  assert.equal(json.found, true);
...
test("plan appearances come back in catalog order", { skip: SKIP_NO_CORE_DB }, async () => {
  ...
  const { json } = await find({ course: "GC", program: GC, catalog_year: YEAR });
  ...
  const lastIdx = wheres.map((w: string) => /Senior/i.test(w)).lastIndexOf(true);
  if (firstIdx !== -1 && lastIdx !== -1) {
    assert.ok(firstIdx < lastIdx, "earlier terms must precede later ones");
  }
});

test("bogus requirement rules are NEVER surfaced (reads the effective view)", { skip: SKIP_NO_CORE_DB }, async () => {
  ...
  assert.equal(json.found, true, "ENSP 2000 is genuinely in the 2021-2022 plan");
