  ...
  sections: [
    {
      ...
      open: true,
      meetings: [
        { days: "TR", beginTime: "1230", endTime: "1345", building: "Godfrey", room: "201", type: "Class" },
      ],
      instructors: [],
    },
  ],
...
test('get-sections-by-crn accepts "Fall 2026" and finds the section', async () => {
  const res = await __schedTools.sectionsByCrn.handler({ term: "Fall 2026", crns: ["80773"] });
  assert.equal(res.isError, undefined, JSON.stringify(res));
  const b = body(res as never);
  assert.equal(b.has_snapshot, true, "a text term must not read as an un-ingested term");
  assert.equal(b.term, "202608", "the resolved code is echoed, not the raw text");
...
test('get-schedule-freshness accepts "Fall 2026"', async () => {
  const res = await __schedTools.scheduleFreshness.handler({ term: "Fall 2026" });
  const b = body(res as never);
  assert.equal(b.has_snapshot, true, "a text term must not report the snapshot missing");
...
test("an UNPARSEABLE term is an error, not an un-ingested term", async () => {
  ...
  for (const bad of ["banana", "2026-08", "Fall 26", "202699"]) {
    const res = await __schedTools.sectionsByCrn.handler({ term: bad, crns: ["80773"] });
    assert.equal(res.isError, true, `${bad} must be an error`);
    assert.match((res.content[0] as { text: string }).text, /Unrecognized term|Accepted forms/);
  }
});
...
test("a VALID term with no snapshot still reports has_snapshot:false, not an error", async () => {
  ...
  const res = await __schedTools.sectionsByCrn.handler({ term: "Spring 2027", crns: ["80773"] });
  assert.equal(res.isError, undefined);
  const b = body(res as never);
  assert.equal(b.has_snapshot, false);
  assert.equal(b.term, "202701", "the resolved code, so the note names a real term");
  assert.deepEqual(b.not_found, [], "nothing was checked, so nothing is claimed");
});
