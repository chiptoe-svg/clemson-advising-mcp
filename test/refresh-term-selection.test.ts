...
test("refreshes the live term AND a published future View-Only term, not past ones", () => {
  assert.deepEqual(
    selectRefreshTerms(TERMS).map((t) => t.code),
    ["202701", "202608"],
  );
});

test("with no live term at all, nothing is refreshed (getTerms degraded)", () => {
  const allViewOnly = TERMS.map((t) => ({
    ...t,
    description: t.description.replace(/ \(View Only\)|$/, " (View Only)"),
  }));
  // every term View Only → maxLive 0 → every code > 0 would match; guard: no live term means no refresh
  const picked = selectRefreshTerms(
    allViewOnly.filter((t) => /View Only/.test(t.description)),
  );
  assert.deepEqual(
    picked.map((t) => t.code),
    [],
